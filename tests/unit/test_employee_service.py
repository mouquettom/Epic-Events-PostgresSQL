...
def make_employee(
    *,
    ...
    """Crée un employé minimal utilisable dans les tests."""
...
def test_create_employee_creates_normalized_employee(
    ...
    hash_password_mock.assert_called_once_with("secret")
    service.repository.get_by_email.assert_called_once_with("jean.dupont@example.com")
...
def test_create_employee_rejects_duplicate_email(
    ...
    with pytest.raises(
        DuplicateError,
        match="Un employé utilise déjà cette adresse email",
    ):
        service.create_employee(
            current_employee=make_employee(role=Role.GESTION),
            first_name="Jean",
...
def test_get_employee_returns_existing_employee(service) -> None:
    current_employee = make_employee(role=Role.GESTION)
    requested_employee = make_employee(employee_id=2)
    service.repository.get_by_id.return_value = requested_employee

    result = service.get_employee(
        current_employee=current_employee,
        employee_id=requested_employee.id,
    )

    assert result is requested_employee
    service.repository.get_by_id.assert_called_once_with(requested_employee.id)
...
def test_get_employee_requires_management_role(
    ...
    with pytest.raises(
        ...
        service.get_employee(current_employee, 2)
...
def test_list_employees_returns_all_employees(service) -> None:
    employees = [
        make_employee(employee_id=1),
        make_employee(employee_id=2),
    ]
    service.repository.get_all.return_value = employees

    result = service.list_employees(make_employee(role=Role.GESTION))
...
def test_update_employee_accepts_same_employee_email(
    service,
    session,
) -> None:
    employee = make_employee(
...
@pytest.mark.parametrize(
    ...
    [
        ("first_name", "  Jeanne  ", "Jeanne"),
        ("last_name", "  Martin  ", "Martin"),
    ],
)
def test_update_employee_trims_name_fields(
    service,
    session,
    field_name,
    value,
...
def test_update_employee_can_assign_empty_trimmed_first_name(
    service,
    session,
) -> None:
    """Documente le comportement actuel : aucune validation à l'update."""
    employee = make_employee(employee_id=2, first_name="Jean")
    service.repository.get_by_id.return_value = employee
    service.repository.update.return_value = employee
...
def test_update_employee_can_assign_empty_normalized_email(
    service,
    session,
) -> None:
    """Documente le comportement actuel : aucune validation à l'update."""
...
def test_delete_employee_deletes_employee_and_commits(
    service,
    session,
) -> None:
    current_employee = make_employee(
    ...
    service.repository.delete.assert_called_once_with(employee_to_delete)
    session.commit.assert_called_once_with()
    session.rollback.assert_not_called()
